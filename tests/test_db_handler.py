...
def test_add_shifts__success(db_handler: DatabaseHandler):
    ...
    for shift in shifts:
        db_handler.add_shift(shift)

    actual_shifts = db_handler.get_shifts("1")
...
def test_update_shift(db_handler: DatabaseHandler):
    ...
    db_handler.update_shift(shift)

    actual_shift = db_handler.get_shifts("1")[0]
...
def test_update_shift__no_change(db_handler: DatabaseHandler):
    ...
    assert len(db_handler.get_shifts("1")) == 2 # at this time should still be 2 shifts
...
def test_delete_shift(db_handler: DatabaseHandler):
    ...
    db_handler.delete_shift("1", datetime.date(year=2024, month=7, day=20))

    assert len(db_handler.get_shifts("1")) == 1
...
def test_delete_employee(db_handler: DatabaseHandler):
    ...
    assert db_handler.get_shifts("1") == []
